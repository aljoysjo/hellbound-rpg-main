...
const ITEM_DATABASE = [
  ...
  { type: 'llave', icon: '🗝️', keywords: ['llave', 'key', 'llaves', 'cerradura'] },
  { type: 'candado', icon: '🔒', keywords: ['candado', 'lock', 'cerradura', 'cerrojo'] },
  { type: 'cuerda', icon: '🪢', keywords: ['cuerda', 'rope', 'soga', 'hilo', 'cable'] },
  { type: 'vidrio', icon: '🔶', keywords: ['vidrio', 'cristal', 'trozo de vidrio', 'vidrio roto', 'fragmento'] },
...
`;
    ...
    // 🎯 NUEVA FUNCIÓN: CONVERTIR TEXTO DE NARRATIVA A ITEM REAL - ARREGLADA
    function convertTextToRealItem(itemText) {
      ...
      // 🔍 BÚSQUEDA MEJORADA: EXACTITUD SOBRE AMPLITUD
      for (const dbItem of ITEM_DATABASE) {
        for (const keyword of dbItem.keywords) {
          const keywordLower = keyword.toLowerCase();
          
          // 🎯 MATCHING MÁS ESTRICTO
          const isExactMatch = textLower === keywordLower;
          const isWordMatch = textLower.split(' ').includes(keywordLower);
          const isContainedWord = new RegExp(`\\b${keywordLower}\\b`).test(textLower);
          
          if (isExactMatch || isWordMatch || isContainedWord) {
            console.log(`✅ Match EXACTO encontrado: "${itemText}" → ${dbItem.type} (keyword: "${keyword}")`);
      ...
      console.log(`❌ Item NO reconocido (no hay match exacto): "${itemText}"`);
    ...
    if (/busco|buscar|examino|examinar|hurgo|hurgar|exploro|explorar|investigo|investigar|descubro|descubrir|registro|registrar|reviso|revisar|miro|mirar|observo|observar|tomo|tomar|agarro|agarrar|recojo|recoger/.test(action.toLowerCase())) {
